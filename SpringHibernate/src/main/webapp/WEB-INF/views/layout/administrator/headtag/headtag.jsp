<%@ page pageEncoding="UTF-8"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<c:set var="contextPath" value="${pageContext.request.contextPath}" />
<!-- Bootstrap 3.3.6 -->
<link rel="stylesheet" href="${contextPath }/css/bootstrap.min.css">
<!-- Font Awesome -->
<link rel="stylesheet" href="${contextPath }/css/font-awesome.min.css">
<!-- Ionicons -->
<link rel="stylesheet" href="${contextPath }/css/ionicons.min.css">

<!-- AdminLTE Skins. We have chosen the skin-blue for this starter
        page. However, you can choose any other skin. Make sure you
        apply the skin class to the body tag so the changes take effect.
        
  -->
<!-- iCheck for checkboxes and radio inputs -->
<link rel="stylesheet" href="${contextPath }/css/plugins/iCheck/all.css">
<!-- daterange picker -->
<link rel="stylesheet"
	href="${contextPath }/css/plugins/daterangepicker/daterangepicker.css">
<!-- bootstrap datepicker -->
<link rel="stylesheet"
	href="${contextPath }/css/plugins/datepicker/datepicker3.css">

<!-- Bootstrap time Picker -->
<link rel="stylesheet"
	href="${contextPath }/css/plugins/timepicker/bootstrap-timepicker.min.css">
<!-- Select2 -->
<link rel="stylesheet"
	href="${contextPath }/css/plugins/select2/select2.min.css">

 <link rel="stylesheet"
	href="${contextPath }/css/dataTables.bootstrap.css"> 
<link rel="stylesheet" href="${contextPath }/css/_all-skins.min.css">
<!-- Theme style -->
<link rel="stylesheet" href="${contextPath }/css/AdminLTE.min.css">
<link rel="stylesheet" href="${contextPath }/css/skin-blue.min.css">
<link rel="stylesheet" href="${contextPath }/css/style.css">
<style>

	tr td:last-child {
		text-align: center;
	}
</style>